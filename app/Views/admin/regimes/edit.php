...
<h1 class="admin-title" style="margin-bottom:1.5rem">Éditer régime</h1>
<form method="post" action="<?= base_url('admin/regimes/update/' . $regime['id']) ?>" class="admin-form">
    <?= csrf_field() ?>
    <div style="margin-bottom:1rem"><label class="form-label">Nom</label><input type="text" name="nom" class="form-control" value="<?= esc($regime['nom']) ?>" required></div>
    <div style="margin-bottom:1rem"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="3"><?= esc($regime['description']) ?></textarea></div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;margin-bottom:1rem">
        <div><label class="form-label">% Viande</label><input type="number" name="pct_viande" class="form-control" id="pct_viande" min="0" max="100" oninput="verifierTotal()" value="<?= (int)$regime['pct_viande'] ?>" required></div>
        <div><label class="form-label">% Poisson</label><input type="number" name="pct_poisson" class="form-control" id="pct_poisson" min="0" max="100" oninput="verifierTotal()" value="<?= (int)$regime['pct_poisson'] ?>" required></div>
        <div><label class="form-label">% Volaille</label><input type="number" name="pct_volaille" class="form-control" id="pct_volaille" min="0" max="100" oninput="verifierTotal()" value="<?= (int)$regime['pct_volaille'] ?>" required></div>
    </div>
    <div style="font-size:.8rem;color:rgba(226,232,240,.5);margin-bottom:1rem">Total: <span id="total-pct">0</span>%</div>
    <div style="margin-bottom:1rem"><label class="form-label">Variation poids / semaine</label><input type="number" name="variation_poids" class="form-control" step="0.01" value="<?= esc($regime['variation_poids_par_semaine']) ?>" required></div>
    <div style="margin-bottom:1.5rem"><label class="form-label">Actif</label><select name="actif" class="form-select"><option value="1" <?= (int)$regime['actif']===1?'selected':'' ?>>Oui</option><option value="0" <?= (int)$regime['actif']===0?'selected':'' ?>>Non</option></select></div>
    ...
    <div id="prix-container">
        ...
        <div class="prix-row" style="display:grid;grid-template-columns:1fr 1fr auto;gap:.8rem;margin-bottom:.8rem">
            <input name="durees[]" type="number" class="form-control" value="<?= (int)$prix['duree_semaines'] ?>">
            <input name="prix[]" type="number" step="0.01" class="form-control" value="<?= (float)$prix['prix'] ?>">
